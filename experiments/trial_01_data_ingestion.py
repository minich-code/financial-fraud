...
from src.utils.commons import create_directories, read_yaml
...
class DataIngestion:
    ...
    def ingest(self) -> Path:
        ...
        try:
            ...
            if df.empty:
                logger.warning("Source file contains no data")
                return None
            ...
            raise Exception(e, category=ErrorCategory.DATA_INGESTION)
    ...
    def _clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            raise Exception(e, category=ErrorCategory.DATA_TRANSFORMATION)

    def _save_data(self, df: pd.DataFrame) -> Path:
        """Save cleaned data to parquet format."""
        try:
            output_path = Path(self.config.root_dir) / self.config.output_file
